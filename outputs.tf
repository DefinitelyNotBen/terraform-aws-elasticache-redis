...
output "endpoint" {
  value       = value       = var.cluster_mode_enabled ? join("", compact(aws_elasticache_replication_group.default.*.configuration_endpoint_address)) : join("", compact(aws_elasticache_replication_group.default.*.primary_endpoint_address))
